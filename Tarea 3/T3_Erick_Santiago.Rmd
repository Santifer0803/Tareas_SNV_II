...
Para este método requerimos de las primas acumuladas en el año y, ya que no se menciona información sobre estas explícitamente, se calculan a través del ratio de siniestralidad (LR). Vea que:

$$LR = \frac{Siniestros_i}{Primas_i} \implies Primas_i = \frac{Siniestros_i}{LR}$$

Es decir, a través del ratio de siniestralidad del $80\%$ y los siniestros (que podemos calcular), podemos obtener las primas brutas ($\mu$) necesarias.

```{r calculo_mu}
# Se calculan las primas correspondientes
primas_brutas <-
  as.vector(tapply(ejercicio_1$monto_siniestro, ejercicio_1$ocurrencia, sum)) / 0.8

# Se muestran las primas brutas
primas_brutas

# Se obtiene el promedio de las primas
primas_brutas <- mean(primas_brutas)
```

Como no hay mucha información del último año de ocurrencia, la prima obtenida es excesivamente baja, por lo que se decide usar un promedio de las primas como $\mu$.
...
(
  data.frame(
    "indice" = 0:8,
    "prima_bf" = rep(mean(primas_brutas), 9),
...
En el gráfico anterior, como se mencionó en su momento, la prima usada para calcular el método de Bornhuetter Ferguson se tomó constante debido al bajo valor de la prima individual en el último año de ocurrencia. Por otro lado, este comportamiento irregular se puede apreciar en el método de Cape Cod, mientras que las primas muestran un comportamiento creciente hasta el tercer año de ocurrencia, posteriormente derecen, llegando a una diferencia de más de 3 millones de colones en el periodo 7 y de más de 13 millones en el último periodo.

Lo anterior causa que el método de Cape Cod sobreestime las primas del primer al sexto periodo, sin embargo, subestima de manera drástica en el séptimo y en el octavo periodo, con respecto al método de Bornhuetter Ferguson. Esta diferencia se resalta en el último periodo, pues las primas recolectadas estimadas son menores a 2 millones de colones, lo cual parece poco realista, pues la siguiente prima más baja supera los 11 millones de colones.
